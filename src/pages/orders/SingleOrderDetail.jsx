...
const SingleOrderDetail = ({ order }) => {
    ...
    const updateOrder = async () => {
        try {
            let response = await doPUT(ORDER_ENDPOINTS.UPDATE(id), { status: currentOrderStatus });
            if (response?.data?.status >= 400) {
                return error(response?.data?.message)
            }
                    ...
                    <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
                        <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                            <div className="flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                                ...
                                <div className="mt-4 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                                        ...
                                        return (
                                            ...
                                            <div key={oIndex} className="bg-white border-b border-gray-200 md:flex-row flex-col flex items-start justify-between w-full p-2 mb-4 space-y-4 md:space-y-0">
                                                ...
                                                <div className="flex justify-between space-x-8 items-center w-full">
                                                    ...
                                                    <p className="text-lg font-semibold leading-6 text-gray-800">Rs {oitem?.price * oitem?.quantity} </p>
                                                </div>
                                            </div>
