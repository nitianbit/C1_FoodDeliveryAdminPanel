...
const SingleOrderDetail = ({ order }) => {
    ...
    const updateOrder = async () => {
        try {
            let response = await doPUT(ORDER_ENDPOINTS.UPDATE(id), { status: currentOrder?.status });
                    ...
                    <div className='space-y-2'>
                        ...
                        <div className="flex items-center gap-5 ">
                            {currentOrder?.status === "Delivered" ?
                                <div variant="outlined" size='sm' color='orange'>{currentOrder?.status}</div>
                                :
                                <Button onClick={updateOrder} variant="outlined" size='sm' color='green'>{currentOrder?.status}</Button>
                            }
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                            ...
                            <div className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                                {currentOrder?.items?.map((oitem, oIndex) => {
                                    return <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                                        <div className="w-full flex flex-col justify-start items-start space-y-8">
                                            <div className="flex justify-start items-start flex-col space-y-2">
                                                <h3 className="text-xl  xl:text-2xl font-semibold leading-6 text-gray-800">{oitem?.name}</h3>
                                            </div>
                                        </div>
                                        <div className="flex justify-between space-x-8 items-start w-full">
                                            {/* <p className="text-base  xl:text-lg leading-6">$36.00 <span className="text-red-300 line-through"> $45.00</span></p> */}
                                            <p className="text-base  xl:text-lg leading-6 text-gray-800">{oitem?.quantity ?? 0}</p>
